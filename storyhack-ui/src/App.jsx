...
export default function App() {
  ...
  async function handleApproveSummary(approvedSummary, approvedFocus) {
    ...
    try {
      ...
      const data = await approveSummary('', approvedSummary, approvedFocus, uploadedLevel)
  ...
  if (videoReady)
    pills.push({ label: '✓ Video ready', color: 'bg-green-100 text-green-700' })
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-3">
          ...
          {pills.length > 0 && (
            <div className="ml-auto flex items-center gap-2 text-xs">
              {pills.map((p) => (
                <span key={p.label} className={`${p.color} px-2.5 py-1 rounded-full font-medium`}>
                  {p.label}
                </span>
              ))}
            </div>
          ...
          <>
            ...
            {loadingSummary && (
              <div className="bg-white rounded-2xl shadow-md p-8 flex flex-col items-center gap-4">
                <svg className="w-10 h-10 text-purple-400 spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" d="M12 3a9 9 0 1 0 9 9" />
                </svg>
                <div className="text-center">
                  <p className="font-semibold text-gray-700">Analysing document…</p>
                  <p className="text-xs text-gray-400 mt-1">Generating summary and core focus</p>
                </div>
              </div>
            )}
            {summaryError && (
