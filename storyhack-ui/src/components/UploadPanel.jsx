...
export default function UploadPanel({ onGenerate, loading }) {
  ...
  function handleSubmit(e) {
    e.preventDefault()
    if (!file) return
    onGenerate(file, level)
  }

  return (
    <div className="bg-white rounded-2xl shadow-md p-6 space-y-5">
      <h2 className="text-lg font-semibold text-gray-700">📄 Upload Document</h2>
      ...
      >
        <svg className="w-10 h-10 text-blue-400 mb-2" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 16.5v-9m0 0-3 3m3-3 3 3M6.75 19.5a4.5 4.5 0 0 1-1.41-8.775 5.25 5.25 0 0 1 10.338-2.04A5.25 5.25 0 0 1 17.25 19.5H6.75z" />
        </svg>
        ...
        <p className="text-xs text-gray-400 mt-1">{ALLOWED_TYPES.join(' · ')}</p>
      </div>
      <input
        id="file-input"
        type="file"
      ...
      <div className="flex items-center gap-3">
        <label className="text-sm font-medium text-gray-600 whitespace-nowrap">Explanation level</label>
        <select
          value={level}
